...
const VALID_SLOTS = ['head', 'chest', 'shoes', 'cape', 'main-hand', 'off-hand'];
    ...
    async showRegearMenu(interaction, db, currentSelections = {}) {
        ...
        if (currentPage === 2) {
            ...
            const createSlotDropdown = (slot) => {
                // For capes, show all tiers (ignore selectedTier filter). For others, use filtered inventory.
                const sourceInventory = (slot === 'cape') ? inventory : filteredInventory;
                const slotItems = sourceInventory.filter(item => item.slot === slot);
            ...
            // Build page 2 rows: Head, Chest, Shoes, Cape in separate rows
            const rows = [
                ...
                new ActionRowBuilder().addComponents(createSlotDropdown('cape'))
            ];

            // Build selected items display for page 2 slots
            const page2Slots = ['head', 'chest', 'shoes', 'cape'];
            ...
            const embed = new EmbedBuilder()
                .setColor('#0099FF')
                .setTitle('🎒 Regear Request - Step 2: Select Armor & Cape')
                .setDescription(`**Tier: ${selectedTier}**${targetUserInfo}\n\nSelect gear for Head, Chest, Shoes, and Cape.`)
