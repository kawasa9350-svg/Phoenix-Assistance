    ...
    async execute(interaction, db) {
        try {
            ...
            if (userMentions) {
                ...
                // Try to parse as comma-separated user IDs, but STRICTLY numeric
                const ids = usersInput.split(',').map(id => id.trim());
                // Filter to keep ONLY valid numeric IDs (simple check for digits)
                const validIds = ids.filter(id => /^\d+$/.test(id));
                userIds.push(...validIds);
            }

            // Deduplicate user IDs to prevent double counting
            ...
            if (userIds.length === 0) {
                const embed = new EmbedBuilder()
                    ...
                    .setDescription('Please provide valid **@User mentions**. \nExample: `@User1, @User2`')
                    .setFooter({ text: 'Phoenix Assistance Bot' })
                    .setTimestamp();
            ...
            // Track total payouts if money was removed
            if (subcommand === 'remove' && updatedUsers.length > 0) {
                // Add individual payout records
                const payoutPromises = updatedUsers.map(userId => 
                    db.addPayout(interaction.guildId, userId, amount, interaction.user.id)
                );
                
                await Promise.all(payoutPromises);
                console.log(`Updated total payouts tracking for ${updatedUsers.length} users`);
            } else {
                console.log(`Skipping payout tracking for subcommand: ${subcommand}`);
            }
