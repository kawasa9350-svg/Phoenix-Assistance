...
class DatabaseManager {
    ...
    async incrementTotalPayouts(guildId, amount) {
        try {
            const collection = await this.getGuildCollection(guildId);
            await collection.updateOne(
                { guildId: guildId },
                { $inc: { totalPayouts: amount } }
            );
            console.log(`✅ Incremented total payouts for guild ${guildId} by ${amount}`);
            return true;
        } catch (error) {
            console.error('❌ Failed to increment total payouts:', error);
            return false;
        }
    }

    async addPayout(guildId, userId, amount, adminId) {
        try {
            const collection = await this.getGuildCollection(guildId);
            const payoutEntry = {
                userId: userId,
            ...
            await collection.updateOne(
                ...
                { 
                    $inc: { totalPayouts: amount },
                    $push: { payoutHistory: payoutEntry }
                }
            );
            console.log(`✅ Added payout for user ${userId} in guild ${guildId}`);
            return true;
        } catch (error) {
    ...
    async getPayoutHistory(guildId, page = 1, limit = 10) {
        try {
            const collection = await this.getGuildCollection(guildId);
            const guild = await collection.findOne({ guildId: guildId });
            
            if (!guild || !guild.payoutHistory) return { entries: [], total: 0 };
            
            // Return entries for the specific page, reversed (newest first)
            const reversedHistory = guild.payoutHistory.reverse();
            const startIndex = (page - 1) * limit;
            const endIndex = startIndex + limit;
            
            return {
                entries: reversedHistory.slice(startIndex, endIndex),
    ...
    async getTotalPayouts(guildId) {
        try {
            const collection = await this.getGuildCollection(guildId);
            const guild = await collection.findOne({ guildId: guildId });
            return guild ? guild.totalPayouts || 0 : 0;
        } catch (error) {
    ...
    async resetTotalPayouts(guildId) {
        try {
            const collection = await this.getGuildCollection(guildId);
            await collection.updateOne(
                { guildId: guildId },
            ...
            console.log(`✅ Reset total payouts for guild ${guildId}`);
            return true;
        } catch (error) {
            console.error('❌ Failed to reset total payouts:', error);
            return false;
        }
    }
    ...
    async removeGearFromInventory(guildId, gearName, slot, tierEquivalent, quantity = 1) {
        try {
            ...
            if (gear.quantity <= quantity) {
                ...
                console.log(`✅ Set gear quantity to 0 in inventory: ${gearName}`);
                return { success: result.modifiedCount > 0, remainingQuantity: 0 };
            } else {
                const result = await collection.updateOne(
                    {
                        guildId: guildId,
                ...
                return { success: result.modifiedCount > 0, remainingQuantity: remainingGear.quantity };
            }
        } catch (error) {
    ...
    async resetAllInventoryQuantities() {
        try {
            ...
            for (const collectionInfo of collections) {
                if (!collectionInfo.name.startsWith('guild_')) continue;
                ...
                if (result.modifiedCount && result.modifiedCount > 0) {
                    console.log(`🧹 Set quantity=0 for ${result.modifiedCount} inventory items in ${collectionInfo.name}`);
                    totalUpdated += result.modifiedCount;
                }
            }

            console.log(`🧮 Total inventory items set to 0: ${totalUpdated}`);
            return totalUpdated;
        } catch (error) {
            console.error('❌ Failed to reset all inventory quantities:', error);
            return 0;
        }
    }
    ...
    async deleteOldRegearReservations(days = 30) {
        try {
            ...
            for (const collectionInfo of collections) {
                ...
                if (result.deletedCount && result.deletedCount > 0) {
                    console.log(`🧹 Deleted ${result.deletedCount} old regear logs from ${collectionInfo.name}`);
                    totalDeleted += result.deletedCount;
                }
            }

            console.log(`🧮 Total old regear logs deleted: ${totalDeleted}`);
            return totalDeleted;
        } catch (error) {
            console.error('❌ Failed to delete old regear reservations:', error);
            return 0;
        }
    }

    async deleteInventoryByTier(tierEquivalent) {
        try {
            const db = this.client.db(this.config.database.databaseName);
            const collections = await db.listCollections().toArray();
            let totalDeleted = 0;
            ...
            for (const collectionInfo of collections) {
                ...
                if (result.deletedCount && result.deletedCount > 0) {
                    console.log(`🗑️ Deleted ${result.deletedCount} inventory items with tier ${tierEquivalent} from ${collectionInfo.name}`);
                    totalDeleted += result.deletedCount;
                }
            }

            console.log(`🧮 Total inventory items deleted for tier ${tierEquivalent}: ${totalDeleted}`);
            return totalDeleted;
        } catch (error) {
            console.error('❌ Failed to delete inventory by tier:', error);
            return 0;
        }
    }
}
