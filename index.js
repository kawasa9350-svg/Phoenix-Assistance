...
const client = new Client({
    intents: [
        ...
        GatewayIntentBits.MessageContent        // Required to read message content for games like /scramble
    ]
});
...
// Event signups loading function removed
...
// Message delete event handler removed
...
client.handleButtonInteraction = async (interaction) => {
    ...
    } else if (customId === 'payout_reset') {
        console.log('Payout reset button detected, routing to payout-tracker command');
        
        const payoutCommand = client.commands.get('payout-tracker');
        if (payoutCommand && payoutCommand.handleButtonInteraction) {
            await payoutCommand.handleButtonInteraction(interaction, payoutCommand.dbManager);
        } else {
            console.error('Payout-tracker command or handleButtonInteraction method not found');
        }
    } else if (customId.startsWith('payout_prev_') || customId.startsWith('payout_next_')) {
        console.log('Payout pagination button detected, routing to payout-tracker command');
...
async function handleAllianceLootsplitWebhook(req, res) {
    try {
        ...
        console.log(`[AllianceWebhook] Received payload:`, JSON.stringify(payload));
        const {
            guildId,
            contentType,
            totalLoot,
        ...
        if (!guildId || !contentType || totalLoot === undefined || !callerId || !Array.isArray(participants)) {
        ...
        //    Then add Untaxed Silver Bags.
        
        const splitPool = (taxableBase - taxAmount) + silverBags;
